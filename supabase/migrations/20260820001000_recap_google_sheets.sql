-- ============================================================================
-- TRUST AI — Migration 10 : RÉCAPITULATIF GOOGLE SHEETS (phase 2)
-- ----------------------------------------------------------------------------
-- Le fichier récapitulatif est la SOURCE DE DISPONIBILITÉ des marchandises.
-- TRUST AI le lit en LECTURE SEULE : il n'écrit jamais dans le Google Sheet
-- (seul l'Apps Script installé dans le fichier gère la colonne « ID TRUST »).
--
-- Cette migration ajoute :
--   * le suivi des lignes disparues du fichier (jamais de suppression) ;
--   * la configuration d'une source (mapping de colonnes) via RPC ;
--   * l'import IDEMPOTENT des lignes, atomique et transactionnel ;
--   * la lecture paginée des lignes et de leur historique.
--
-- Aucune table logistique n'est lisible directement : tout passe par des
-- fonctions `security definer` qui vérifient permission ET organisation
-- (résolue depuis auth.uid(), jamais reçue du navigateur).
--
-- Rejouable et transactionnelle, comme la migration 9.
-- ============================================================================
...
begin
  if exists (
    ...
    raise notice 'TRUST AI : migration 10 déjà appliquée — réexécution sans effet.';
  end if;
end $$;

-- ===========================================================================
-- 1. Suivi des lignes disparues du fichier
-- ---------------------------------------------------------------------------
-- Une ligne retirée du récapitulatif n'est JAMAIS supprimée : elle est
-- marquée absente et signalée pour contrôle humain.
-- ===========================================================================

alter table public.logistics_lines
  add column if not exists missing_since timestamptz,
  add column if not exists last_read_id uuid references public.recap_reads(id);

create index if not exists logistics_lines_missing_idx
  on public.logistics_lines (organization_id)
  where missing_since is not null;

-- ===========================================================================
-- 2. Configuration d'une source de récapitulatif
-- ===========================================================================
...
begin
  ...
  v_label := coalesce(nullif(btrim(p_payload->>'label'), ''), 'Récapitulatif');

  if coalesce(nullif(btrim(p_payload->>'spreadsheet_id'), ''), '') = '' then
    raise exception 'L''identifiant du Google Sheet est obligatoire.';
  end if;
  if coalesce(nullif(btrim(p_payload->>'sheet_name'), ''), '') = '' then
  ...
  if v_id is null then
    ...
    set spreadsheet_id = btrim(p_payload->>'spreadsheet_id'),
        sheet_name = btrim(p_payload->>'sheet_name'),
  ...
  perform app.log_activity(
    ...
    v_label || ' — onglet « ' || btrim(p_payload->>'sheet_name') || ' ».');
  return v_id;
end;
$$;

-- ===========================================================================
-- 3. Import IDEMPOTENT des lignes du récapitulatif
-- ---------------------------------------------------------------------------
-- Reçoit les lignes DÉJÀ analysées par le serveur applicatif (le parsing du
-- Google Sheet reste côté Node, testable unitairement) et applique en une
-- seule transaction :
--   * identification par (source_id, recap_row_id) — l'« ID TRUST » — sinon
--     par empreinte déterministe de secours ;
--   * création, mise à jour ou constat d'absence de changement ;
--   * événements logistiques sans doublon (une réception n'est enregistrée
--     qu'une fois) ;
--   * anomalies pour les cas ambigus ;
--   * marquage (jamais suppression) des lignes disparues du fichier.
-- ===========================================================================
  ...
  loop
    ...
    if v_id is null then
      insert into public.logistics_lines (
        ...
        destination_confidence, raw_row, created_by, last_read_id)
      values (
        ...
        v_dest, v_conf, v_row->'raw_row', v_profile, v_read)
      returning id into v_id;
    ...
    -- Anomalies signalées par le parseur (destination ambiguë, dates
    -- incohérentes, données manquantes…) : jamais de doublon non résolu.
...
-- ===========================================================================
-- 4. Lecture des lignes logistiques (aucun accès direct aux tables)
-- ===========================================================================
